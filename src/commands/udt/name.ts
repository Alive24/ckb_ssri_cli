...
export default class UDTName extends Command {
  ...
  public async run(): Promise<void> {
    const {args, flags} = await this.parse(UDTName)
    // Method path hex function
    const hasher = new HasherCkb()
    const namePathHex = hasher.update(Buffer.from('UDT.name')).digest().slice(0, 18)
